...
class WaveSocket
	...
	def recv(nonblock=false)
		Packet.parse @sock, nonblock
	end
...
class Packet
	...
	def self.parse(sock, nonblock=false)
		...
		if nonblock
			bytes = ''
			begin
				bytes = sock.read_nonblock(1)
			rescue Errno::EAGAIN
				#sock.ungetc bytes[0]
				return nil
			end
			bytes << sock.read(3)
			data = StringIO.new(sock.read(bytes.unpack('I').first))
		else
...
class Wave
	attr_accessor :id, :domain, :participants, :messages, :revisions, :digest
...
class Revision
	...
	attr_accessor :added_participants, :removed_participants, :deltas, :message
